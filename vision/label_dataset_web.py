...
fetch('/api/groups').then(r=>r.json()).then(groups=>{const root=document.querySelector('#groups');groups.forEach(g=>{const card=document.createElement('section');card.className='card';const options=(selected)=>`<option value='' disabled ${selected?'':'selected'}>请选择</option>`+labels.map(x=>`<option ${x===selected?'selected':''}>${x}</option>`).join('');card.innerHTML=`<h2>组 ${String(g.index).padStart(2,'0')}（${g.count} 张）</h2><p>${g.first}<br>至 ${g.last}</p><img src='${g.image}'><div class='controls'>${['left','center','right'].map(name=>`<label>${name}<select data-box='${name}'>${options(g.labels[name]||'')}</select></label>`).join('')}</div><button class='save'>保存本组</button><span class='saved'></span>`;card.querySelector('.save').onclick=()=>{const values={};card.querySelectorAll('select').forEach(s=>values[s.dataset.box]=s.value);fetch('/api/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index:g.index,labels:values})}).then(r=>r.json()).then(x=>{card.querySelector('.saved').textContent=x.ok?'已保存':'保存失败';});};root.appendChild(card);});});
</script></body></html>"""
